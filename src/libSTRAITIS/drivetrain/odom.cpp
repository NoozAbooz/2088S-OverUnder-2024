...
double get_dt_distance_travelled() { 
	...
	double distance = (avg_position / 360) * wheel_circumference * (42.0/48.0);
	return distance;
}
...
void strait::odomThread() {
	...
	while (true) {
		...
		// print for debugging
		chassis.setPose(chassis.getPose().x, chassis.getPose().y, get_imu_heading());
